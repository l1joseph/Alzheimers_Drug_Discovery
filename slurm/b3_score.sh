...
WORK=$SCRATCH/b1_score_$SLURM_JOB_ID
...
echo "=== B1 scoring (7 PKU repurposing candidates) at $(date) ==="

cd $PROJECT
boltz predict $PROJECT/data/tamgen_b3_yamls --no_kernels \
  --cache $BOLTZ_CACHE --out_dir $WORK \
  --output_format mmcif --accelerator gpu --devices 1

mkdir -p $PROJECT/results/tamgen_b3
python $PROJECT/scripts/aggregate_boltz.py \
    --pred-dir $WORK \
    --out-csv $PROJECT/results/tamgen_b3/scores.csv
...
addrows(f'{P}/results/tamgen_b3/scores.csv', 'tamgen_b3')
